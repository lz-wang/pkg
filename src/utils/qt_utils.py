...
def reconnect(signal, slot):
    """disconnect all slots of this signal and connect current slot(s)"""
    try:
        signal.disconnect()
...
def set_label_text(label: QLabel, text: str, level: str = 'INFO'):
    ...
    Args:
        label (QLabel): QLabel
        text (str): text string to set
        level (str, optional): a map for QLabel color. Defaults to 'INFO'.
    """
...
def show_qthread_status(qt: QThread, name: str = None, delay: float = 0, log_level: str = 'INFO'):
    ...
    Args:
        qt: target QThread
        name: custom name of QThread
        delay: after delay seconds to show_qthread_status
        log_level: log level
    """
    if delay > 0:
        time.sleep(delay)
